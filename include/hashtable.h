...
struct HashTable {
    ...
    T get(const string& key); // ф-ия получения элемента по ключу
    bool find(const string& key); // ф-ия поиска элемента по ключу
    bool remove(const string& key); // ф-ия удаления элемента по ключу
    bool replace(const string& key, T newValue); // ф-ия замены элемента по ключу
};
